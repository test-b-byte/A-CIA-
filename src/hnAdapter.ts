// src/hnAdapter.ts
// Stage 2 adapter for Hacker News. Converts raw JSON into PaperRecord[].
...
export function parseHnResponse(rawJsonText: string): PaperRecord[] {
...
// MARKER: HN's front page is general tech news, not filtered to AI or ML.
  ...
  return parsedResponse.hits
    .filter((hit: any) => hit && hit.objectID && isNonEmptyText(hit.title ?? ""))
    .map((hit: any): PaperRecord => {
      return {
